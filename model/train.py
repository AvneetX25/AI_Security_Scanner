...
# This default is used during training eval; the threshold sweep will refine it.
THRESHOLD  = 0.35
...
#
# FIX #3: alpha_t is now cast to match ce's dtype+device to avoid float32/float64
#          mismatch crashes (especially on some PyTorch + CUDA version combos).
class FocalLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        # Cast scalars to match ce tensor's dtype and device — avoids float64 mismatch
        alpha_t = torch.where(
            targets == 1,
            torch.tensor(self.alpha,         dtype=ce.dtype, device=ce.device),
            torch.tensor(1.0 - self.alpha,   dtype=ce.dtype, device=ce.device),
        )
...
# ── Train / val split ─────────────────────────────────────────────────────────
# FIX #5: Use stratified split so both splits have the same vulnerable/safe ratio.
#          Random sampling on imbalanced data can accidentally starve val of rare
#          vulnerable samples, making recall metrics meaningless.
print("\n[2/5] Splitting dataset (stratified by label)...")
...
print(f"  Train : {len(train_df)} rows  "
      f"(vuln={( train_df['label']==1).sum()}, safe={(train_df['label']==0).sum()})")
print(f"  Val   : {len(val_df)} rows  "
      f"(vuln={(val_df['label']==1).sum()}, safe={(val_df['label']==0).sum()})")
...
#
# FIX #1: class_weights was previously computed but never used anywhere.
#          Now logged explicitly. If you want to pass it into FocalLoss instead
#          of the fixed alpha, replace alpha with weight_vuln/(weight_safe+weight_vuln).
...
# Derive FocalLoss alpha directly from class weights so they stay in sync.
# alpha = vuln_weight / (safe_weight + vuln_weight) keeps it in [0, 1].
focal_alpha = weight_vuln / (weight_safe + weight_vuln)

print(f"  Class weights → Safe: {weight_safe:.3f} | Vulnerable: {weight_vuln:.3f}")
print(f"  Derived focal alpha  : {focal_alpha:.3f}")
...
training_args = TrainingArguments(
    ...
    eval_strategy               = "epoch",
    save_strategy               = "epoch",
    save_total_limit            = 2,          # FIX #6: prevent disk bloat on longer runs
...
# FIX #2: FocalLoss is instantiated inside the Trainer class (as self.loss_fn)
#          instead of a global closure. This avoids device mismatch on multi-GPU
#          setups where logits may be on a different device than the outer scope.
class RecallFocusedTrainer(Trainer):
    """Identical to HuggingFace Trainer but uses FocalLoss instead of CE."""

    def __init__(self, *args, focal_alpha: float = 0.75, focal_gamma: float = 2.0, **kwargs):
        super().__init__(*args, **kwargs)
        # Loss fn lives on the trainer — no global closure, safe for multi-GPU
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # Move loss_fn to same device as logits at call time — correct for multi-GPU
        loss = self.loss_fn.to(logits.device)(logits, labels)
...
trainer = RecallFocusedTrainer(
    ...
    focal_alpha     = focal_alpha,   # derived from class weights above
    focal_gamma     = 2.0,
)
...
# ── Train ─────────────────────────────────────────────────────────────────────
print("\nTraining for 3 epochs with Focal Loss + class-weight-derived alpha...")
trainer.train()
...
#
# FIX #4: Iterate HIGH → LOW and pick the HIGHEST threshold where recall ≥ 0.95.
#          The original code iterated low→high and always picked 0.20 (trivially
#          high recall by flagging everything). We want the LEAST aggressive
#          threshold that still satisfies the recall floor — this maximises
...
best_threshold     = THRESHOLD   # fallback if no threshold hits ≥ 0.95
best_threshold_set = False

# High → Low: we stop at the first (= highest) threshold that meets our recall floor
...
if not best_threshold_set:
    print(f"\n  ⚠️  No threshold achieved recall ≥ 0.95. Falling back to THRESHOLD={THRESHOLD}.")
    print(     "     Consider more epochs, stronger alpha, or reviewing label quality.")
else:
    print(f"\n  Best threshold for recall ≥ 0.95 → {best_threshold}")

# ── Classification report at best threshold ───────────────────────────────────
# NOTE: final_preds uses best_threshold (from sweep), NOT the THRESHOLD constant
#       used during training eval. The results_payload below reflects this
#       corrected threshold intentionally — this is the number that matters
#       at inference time.
...
# Save best threshold alongside model so inference.py uses the same value
...
# ── Save results ──────────────────────────────────────────────────────────────
# Metrics in "finetuned" are recomputed at best_threshold (not THRESHOLD=0.35),
# so they accurately reflect inference-time performance.
results_payload = {
    "baseline"  : baseline_metrics,
    "finetuned" : {
        **final_metrics,
        # Override trainer.evaluate() metrics (which used THRESHOLD=0.35)
        # with sweep-derived metrics at best_threshold — this is what inference uses
        "f2_vuln"        : round(fbeta_score(sweep_labels, final_preds,    beta=2, pos_label=1, zero_division=0), 4),
        "recall_vuln"    : round(recall_score(sweep_labels, final_preds,           pos_label=1, zero_division=0), 4),
        "precision_vuln" : round(precision_score(sweep_labels, final_preds,        pos_label=1, zero_division=0), 4),
        "best_threshold" : best_threshold,
        "threshold_note" : "Metrics above are at best_threshold, not THRESHOLD=0.35",
    },
